...
#
# Minor revisions and conversion of the I2C interface to use wiringpi2 by Tom Moore 2014-01-10
# Other than the change in underlying I2C library, none of the changes were intended to change the original
# interfaces in the from the previous version
# Minor changes included
#     simplified the channel and sps selection logic
#     simplified the reading and conversion of ADC values to millivolts
#     simplified the logic for setting the comparison registers
# ===========================================================================
...
class ADS1x15:
  ...
  difChanADS1x15 = {
    1: __ADS1015_REG_CONFIG_MUX_DIFF_0_1,
    3: __ADS1015_REG_CONFIG_MUX_DIFF_0_3,
    13: __ADS1015_REG_CONFIG_MUX_DIFF_1_3,
    23: __ADS1015_REG_CONFIG_MUX_DIFF_2_3
  }
  chanADS1x15 = {
    0: __ADS1015_REG_CONFIG_MUX_SINGLE_0,
    1: __ADS1015_REG_CONFIG_MUX_SINGLE_1,
    2: __ADS1015_REG_CONFIG_MUX_SINGLE_2,
    3: __ADS1015_REG_CONFIG_MUX_SINGLE_3
}
  ...
  def readADCSingleEnded(self, channel=0, pga=6144, sps=250):
    ...
    if (channel not in self.chanADS1x15):
      if (self.debug):
    ...
    if ( (sps not in self.spsDict) & self.debug):  
       print("ADS1x15: Invalid sps specified: %d, using 250sps" % sps) 
    config |= self.spsDict.setdefault(sps, self.__ADS1115_REG_CONFIG_DR_250SPS)
    ...
    config |= self.chanADS1x15[channel]

    # Set 'start single-conversion' bit
    config |= self.__ADS1015_REG_CONFIG_OS_SINGLE

    # Write config register to the ADC
    self.i2c.writeReg16(self.fd, self.__ADS1015_REG_POINTER_CONFIG, config)
    if self.debug:
      reg = self.i2c.readReg16(self.fd,self.__ADS1015_REG_POINTER_CONFIG)
      print("Config reg: %04x" % reg)
    ...
    # Read the conversion results(signed 2s complement 16 bit integer),
    #     extend the sign bit so result has correct sign
    #     adjust for gain and convert to millivolts
    result = self.i2c.readReg16(self.fd, self.__ADS1015_REG_POINTER_CONVERT)
    if self.debug:
      print("reading ch: %i - raw value: %i" % (channel, result))
      print("AD value: %04x" % result)
    result = ((result + 0x8000) & 0xffff) - 0x8000
    return( result * pga / 32768.0)
  ...
  def readADCDifferential(self, chP=0, chN=1, pga=6144, sps=250):
    ...
    #set channels
    if((chp*10+chN) in self.difChanADS1x15):
      config |= self.difChanADS1x15[chP*10+chN]
    else:
      if (self.debug):
  ...
  def startSingleEndedComparator(self, channel, thresholdHigh, thresholdLow, \
    ...
    self.i2c.writeReg16(self.fd, self.__ADS1015_REG_POINTER_HITHRESH, thresholdHighWORD)     
  
    thresholdLowWORD = int(thresholdLow*(32767.0/pga))    
    self.i2c.writeReg16(self.fd, self.__ADS1015_REG_POINTER_LOWTHRESH, thresholdLowWORD)     
  ...
  def startDifferentialComparator(self, chP, chN, thresholdHigh, thresholdLow, \
    ...
    # Write threshold high and low registers to the ADC
    # V_digital = (2^(n-1)-1)/pga*V_analog
    # Note that the bottom 4 bits of the ADS1015 threshold registers are "don't care" bits.
    # that means we can treat the ADS1015 and ADS1115 in the same way. See datasheet page 17
    thresholdHighWORD = int(thresholdHigh*(32767.0/pga))
    self.i2c.writeReg16(self.fd, self.__ADS1015_REG_POINTER_HITHRESH, thresholdHighWORD)     
...
class ADS1015(ADS1x15):
  def __init__(self, address=0x48, debug=False):
    super().__init__(address, 0, debug)
    
class ADS1115(ADS1x15):
  def __init__(self, address=0x48, debug=False):
    super().__init__(address, 1, debug)
